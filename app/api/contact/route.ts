...
export async function POST(req: Request) {
  try {
    ...
    // 1. Protección Anti-Spam: Campo Honeypot (invisible para humanos)
    ...
    // 2. Protección Anti-Spam: trampa de tiempo (envío menor a 1.5s = bot)
    ...
    // El teléfono es opcional; si se proporciona, se valida el formato
    ...
    if (!apiKey) {
      ...
      console.log(`[Contacto simulado] ${name} <${email}> — ${selectedReason}`)
    ...
    await resend.emails.send({
      ...
      to: targetEmail,
      subject: `[Web] Mensaje de ${name} — ${selectedReason}`,
