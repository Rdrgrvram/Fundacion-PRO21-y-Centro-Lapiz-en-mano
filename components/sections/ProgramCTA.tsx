...
export default function ProgramCTA({ lang, program, whatsappNumber }: ProgramCTAProps) {
  const style = EXTENDED_PALETTE[program.hero.color]

  return (
    <section className={`py-16 ${style.bgSolid}`}>
      <div className="container mx-auto px-4 text-center">
        <h2 className={`text-3xl md:text-4xl font-extrabold mb-4 ${style.textOn}`}>{program.cta.title}</h2>
        <p className={`text-lg mb-8 max-w-xl mx-auto ${style.textOn === 'text-white' ? 'text-white/90' : 'text-gray-800'}`}>
          {program.cta.text}
        </p>
