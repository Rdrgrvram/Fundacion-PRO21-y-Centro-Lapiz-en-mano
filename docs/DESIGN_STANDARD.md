...
- Los emoji siguen siendo aceptables como iconografía **decorativa/editorial** proveniente del CMS (valores de la fundación, stats, badges de programas, chips de área) — cambiarlos a un set de SVG requeriría modificar `lib/cms-schemas.ts` y `public/admin/config.yml` (pasar de texto libre a un selector de íconos), más migrar cada entrada de contenido existente. Es un cambio de mayor alcance, pendiente de una ronda dedicada si se decide encarar.
